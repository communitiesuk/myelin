...
#   The trailer is written unless NOTRAILER=1; the ADR path is docs/adr/0002-x.md
...
artefact() {
  ...
  (cd "$dir" && printf '.worktrees/\n' >> .gitignore && g add .gitignore && g commit -q -m "ignore worktrees")
  if [ "$where" = worktree ]; then (cd "$dir" && g switch -q -c topic && printf 'x\n' > t.txt && g add t.txt && g commit -q -m topic); fi
  local out; out="$(cd "$dir" && bash "$SCRIPT" begin "${ADRPATH:-docs/adr/0002-x.md}" 2>&1)" || { echo "$out"; return 2; }
  local wd; wd="$(printf '%s\n' "$out" | sed -n 's/^dir=//p')"
  ( cd "$wd" && mkdir -p "$(dirname "${ADRPATH:-docs/adr/0002-x.md}")" && printf -- '---\ntitle: x\n---\n' > "${ADRPATH:-docs/adr/0002-x.md}" && g add -A \
    && if [ "${NOTRAILER:-0}" = 1 ]; then g commit -q -m "Add ADR"; else g commit -q -m "$(printf 'Add ADR\n\nDerives-From: %s\n' "${TRAILER:-docs/discovery/0001-note.md}")"; fi )
  if [ "${EXTRA:-0}" = 1 ]; then ( cd "$wd" && printf 'more\n' >> "${ADRPATH:-docs/adr/0002-x.md}" && g add -A && g commit -q -m "Extend ADR" ); fi
  echo "$wd"
}
# github-flavoured remote: fetch URL names github.com so host=github, pushes go to the bare repo
githubify() { (cd "$1" && git remote set-url --push origin "$(pwd)/.origin/repo.git" && git remote set-url origin https://github.com/example/repo.git); export GH_STUB_BARE="$1/.origin/repo.git" GH_STUB_TRUNK=dev; }
...
t "refusal: dirty working tree"
  ...
  assert_eq "adr-0002-x" "$(cd "$R/r2" && git rev-parse --abbrev-ref HEAD)" "refusal must change nothing"
...
t "refusal: first commit has no Derives-From trailer"
  ...
  assert_eq "$(cd "$R/r4" && git rev-parse dev)" "$(cd "$R/r4" && git rev-parse dev)" ; assert_eq "1" "$(cd "$R/r4" && git rev-list --count dev..adr-0002-x)"
...
t "decision: a new docs/adr file is proposed, never merged, exit 3"
  wd="$(artefact "$R/d1" --remote origin --where worktree)"; githubify "$R/d1"; : > "$GH_STUB_LOG"
  ...
  assert_eq "0" "$(cd "$R/d1" && git log --merges --format=%H fixture 2>/dev/null | wc -l | tr -d ' ')" ; assert_eq "$(cd "$R/d1" && git rev-parse dev)" "$(bare_tip "$R/d1" dev)"
  assert_eq "1" "$(cd "$R/d1" && git rev-list --count "$(git merge-base dev adr-0002-x)"..dev; echo)" >/dev/null 2>&1; true
...
t "decision: with no remote it cannot be proposed, exit 3, branch kept"
  wd="$(artefact "$R/d3" --where worktree)"
  out="$(land "$wd"; echo "exit=$?")"; assert_contains "$out" "exit=3"
...
t "no remote: merge in place, primary ends on the trunk, two parents, branch gone, no remote invented"
  ...
  assert_eq "0" "$(cd "$R/m1" && git for-each-ref refs/heads/adr-0002-x | wc -l | tr -d ' ')"
...
t "no remote: merge from a worktree with an untracked .venv, worktree removed, dirt untouched, no checkout line"
  ...
  [ -d "$R/m2/.worktrees/adr-0002-x" ] && fail "worktree not removed"
  ...
  assert_eq "0" "$(cd "$R/m2" && grep -c 'checkout: moving from dev' .git/logs/HEAD)"
  assert_eq "0" "$(cd "$R/m2" && git for-each-ref refs/heads/adr-0002-x | wc -l | tr -d ' ')"
...
t "no remote: primary checkout on neither trunk nor branch -> exit 1, nothing merged"
  ...
  [ -d "$R/m3/.worktrees/adr-0002-x" ] || fail "worktree removed without a merge"
...
t "github: merge in place with --merge, local trunk fast-forwarded to origin, cleanup"
  ...
  assert_eq "0" "$(cd "$R/g1" && git for-each-ref refs/heads/adr-0002-x | wc -l | tr -d ' ')"
...
t "github: merge from a worktree, run from the primary checkout, worktree removed"
  ...
  out="$(land "$wd"; echo "exit=$?")"; assert_contains "$out" "exit=0"
  [ -d "$R/g2/.worktrees/adr-0002-x" ] && fail "worktree not removed"
...
t "stop: the host refuses the merge -> pushed, branch and worktree kept, HEAD unchanged, exit 1 with the hint"
  ...
  assert_eq "$(cd "$R/s1" && git rev-parse adr-0002-x)" "$(bare_tip "$R/s1" adr-0002-x)" "pushed at the same SHA"
  assert_eq "1" "$(cd "$R/s1" && git for-each-ref refs/heads/adr-0002-x | wc -l | tr -d ' ')"
  [ -d "$R/s1/.worktrees/adr-0002-x" ] || fail "worktree removed without a merge"
...
t "stop: gh absent from PATH -> pushed, kept, exit 1 naming gh"
  ...
  assert_eq "$(cd "$R/s2" && git rev-parse adr-0002-x)" "$(bare_tip "$R/s2" adr-0002-x)" "pushed at the same SHA"
  assert_eq "1" "$(cd "$R/s2" && git for-each-ref refs/heads/adr-0002-x | wc -l | tr -d ' ')"
  [ -d "$R/s2/.worktrees/adr-0002-x" ] || fail "worktree removed without a merge"

t "stop: an unknown host -> pushed, kept, exit 1"
  wd="$(artefact "$R/s3" --remote origin --where worktree)"
  (cd "$R/s3" && git remote set-url --push origin "$(pwd)/.origin/repo.git" && git remote set-url origin https://gitlab.example.org/x/y.git)
  out="$(land "$wd"; echo "exit=$?")"; assert_contains "$out" "exit=1"; assert_contains "$out" "gitlab.example.org"
  assert_eq "$(cd "$R/s3" && git rev-parse adr-0002-x)" "$(bare_tip "$R/s3" adr-0002-x)" "pushed at the same SHA"
